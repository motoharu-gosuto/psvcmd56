...
{ 
   ...
   if (clearsign_exists)                       
   {
      if (rif_file_path)
      {
         last_error = get_klicensee_23D642C(rif_file_path, &gdat_ctx_copy->rif);
         if (last_error < 0)
         {
            #pragma region
            if (alloc_buffer)
      ...
      {
         last_error = sceAppMgrCheckRifGD_base_23D9704(app_path, gdat_ctx_copy->titleId_378, &gdat_ctx_copy->rif);
         if (last_error < 0)
         {
            #pragma region
            if (alloc_buffer)
   ...
   last_error = proc_parse_param_sfo_23D5028(SceThreadmgrForDriver_ksceKernelGetProcessId_9dcb4b7a(), param_sfo_path, 0, gdat_ctx_copy);
   if(last_error < 0)
   {
      #pragma region
      if(md_mount_point[0])
   ...
   if(patch_path)
   {
      ...
      int stat_res0 = SceIofilemgrForDriver_sceIoGetstatForDriver_75c96d25(patch_path_copy, &stat);// simple check that dir exists
   ...
   {
      int get_patch_res = get_patch_path_23D6A48(app_path, gdat_ctx_copy->titleid_388, gdat_ctx_copy->flag_29C, gdat_ctx_copy->prefix_298, patch_path_copy);
      if (get_patch_res >= 0)
      {
         SceIoStat stat;
