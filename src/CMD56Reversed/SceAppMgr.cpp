...
int create_mount_with_klicensee(SceUID pid, mount_ctx_t *mctx_alloc0, mount_ctx_holder_t *mount_ctx_holder, const char *physical_path, const char* mount_drive_input, mount_point_data_entry *mpd_entry_alloc0, SceUInt64 auth_id, const char* klicensee, std::uint16_t mode_index, char *gen_mount_point)
{
   char klicensee0[16];
   memcpy(klicensee0, klicensee, 0x10);
   return create_pfs_mount_raw(pid, mpd_entry_alloc0->mount_id, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, gen_mount_point, mpd_entry_alloc0, auth_id, klicensee0, mode_index, mpd_entry_alloc0->path, mpd_entry_alloc0->gen_mount_point);
}

int create_mount_with_empty_klicensee(SceUID pid, mount_ctx_t *mctx_alloc0, mount_ctx_holder_t *mount_ctx_holder, const char *physical_path, const char* mount_drive_input, mount_point_data_entry *mpd_entry_alloc0, SceUInt64 auth_id, std::uint16_t mode_index, char *gen_mount_point)
{
   char klicensee0[16] = {0};
   return create_pfs_mount_raw(pid, mpd_entry_alloc0->mount_id, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, gen_mount_point, mpd_entry_alloc0, auth_id, klicensee0, mode_index, mpd_entry_alloc0->path, mpd_entry_alloc0->gen_mount_point);
}

int create_mount_from_sealedkey(SceUID pid, mount_ctx_t *mctx_alloc0, mount_ctx_holder_t *mount_ctx_holder, const char *physical_path, const char* mount_drive_input, mount_point_data_entry *mpd_entry_alloc0, SceUInt64 auth_id, std::uint16_t mode_index, char *gen_mount_point)
{
   char klicensee0[16] = {0}; 
   ...
   {
      return mpd_cleanup(pid, mpd_entry_alloc0->mount_id, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, gen_mount_point, mpd_entry_alloc0, sk_read_res != 0x80800003, sk_read_res);
   }
}

int create_mount_from_klicensee_or_sealedkey(SceUID pid, mount_ctx_t *mctx_alloc0, mount_ctx_holder_t *mount_ctx_holder, const char *physical_path, const char* mount_drive_input, mount_point_data_entry *mpd_entry_alloc0, SceUInt64 auth_id, const char* klicensee, std::uint16_t mode_index, char *gen_mount_point)
{
   char klicensee0[16] = {0};
...
int create_mountpoint_core(SceUID pid, mount_ctx_holder_t *mount_ctx_holder, mount_ctx_t *mctx_alloc0, mount_point_data_entry *mpd_entry_alloc0, const char *physical_path, const char* mount_drive_input, SceUInt64 auth_id, const char* klicensee, char *mount_point_result)
{   
   if ((mpd_entry_alloc0->mount_id < 0x64))
   {
      return mpd_cleanup(pid, mpd_entry_alloc0->mount_id, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mount_point_result, mpd_entry_alloc0, 1, 0x80800001);
   }
   else if((mpd_entry_alloc0->mount_id >= 0x64) && (mpd_entry_alloc0->mount_id <= 0x6A))
   {
      return create_mount_with_empty_klicensee(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, 0x16, mount_point_result); //fake / redirect
   }
   else if ((mpd_entry_alloc0->mount_id >= 0x6B) && (mpd_entry_alloc0->mount_id <= 0x6C))
   {
      return create_mount_with_empty_klicensee(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, 0x15, mount_point_result); //fake / redirect
   }
   ...
   {
      return create_mount_from_klicensee_or_sealedkey(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, klicensee, 0x05, mount_point_result); //savedata like
   }
   ...
   {
      return create_mount_from_sealedkey(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, 0x06, mount_point_result); //savedata like
   }
   ...
   {
      return create_mount_from_sealedkey(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, 0x04, mount_point_result); //ac
   }
      ...
      case 0x3E9:
         {
            ...
            {
               return create_mount_with_klicensee(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, klicensee, 0x02, mount_point_result); //game
            }
            else
            {
               return create_mount_with_empty_klicensee(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, 0x14, mount_point_result); //fake / redirect
            }
         }
      case 0x3EA:
      case 0x3EB:
         {
            ...
            return create_mount_from_sealedkey(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, mode_index, mount_point_result);
         }
      case 0x3EC:
         return create_mount_with_klicensee(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, klicensee, 0x0D, mount_point_result); //game
      case 0x3ED:
         return create_mount_from_klicensee_or_sealedkey(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, klicensee, 0x05, mount_point_result); //savedata like
      case 0x3EE:
         return create_mount_with_empty_klicensee(pid, mctx_alloc0, mount_ctx_holder, physical_path, mount_drive_input, mpd_entry_alloc0, auth_id, 0x15, mount_point_result); //fake / redirect
...
int create_mountpoint_base_23D9B50(SceUID pid, mount_ctx_holder_t *mount_ctx_holder, unsigned int mount_id, char *title_id, char *physical_path, char *mount_drive, char *klicensee, char *mount_point_result)
{
   ...
   return create_mountpoint_core(pid, mount_ctx_holder, mctx_alloc0, mpd_entry_alloc0, physical_path_local, mount_drive_input, auth_id, klicensee, mount_point_result);
}
